package com.example.nrbzms17.ui.view;
...
public class SideslipListView extends ListView {
    ...
    private boolean performActionMove(MotionEvent ev) {
        ...
        int nowY = (int) ev.getY();
        int diffX = nowX - mDownX;
        if (mPointChild != null && mItemLayoutParams!= null && Math.abs(diffX) > Math.abs(nowY - mDownY) && Math.abs(nowY - mDownY) < 20) {
    ...
    private void performActionUp(MotionEvent ev) {
        ...
        if (mPointChild != null && mItemLayoutParams != null && -mItemLayoutParams.leftMargin >= mDeleteWidth / 2) {
    ...
    public void turnNormal() {
        if(mPointChild != null && mItemLayoutParams != null) {
